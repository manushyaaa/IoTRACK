...
void setup() {
  ...
  digitalWrite(A14, HIGH); // PIN 15 BLA (Positive)
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (Serial2.available() > 0) {
      ...
      if (data.length() >= elePos) {
        ...
        lcd.print("Date: " + date);
        lcd.setCursor(0, 2);
        lcd.print("Azi: " + String(ele, 2)); // Display azi with 2 decimal places
        lcd.setCursor(0, 3);
        lcd.print("Ele: " + String(azi, 2)); // Display ele with 2 decimal places
      }
    }
  }
}
