...
import json
from utils import colorize, colorize_output , generatePos
    ...
    while True:
        ...
        try:
            while True:
                ...
                azi , ele = generatePos(satName , location)

                packet = f'{current_time}_{date}_{azi}_{ele}\n'
                packet_console = f'{current_time}_{date}_{azi}_{ele}'
                print("TX : " , colorize_output(packet_console, "white" , "on_blue"))
...
def SerialListener():
    ...
    try : 
        while True:
            ...
            time_str, date_str, ele, azi = data.split("_")
 
            ele = float(ele)
            azi= float(azi)
           
            packet = f'{time_str} {date_str} {ele} {azi}'

            print("RX : " ,colorize_output(  packet, "green" , "on_white"))       
